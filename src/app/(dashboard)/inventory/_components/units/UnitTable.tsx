...
import { restoreUnit, deleteUnit } from '@/services/UnitService';
...
export default function UnitTable({
  ...
  const [confirmTarget, setConfirmTarget] = useState<Unit | null>(null);
  ...
  const handleDeleteConfirm = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteUnit(confirmTarget.id, isTrashed);
      toast.success(isTrashed ? 'Unit permanently deleted' : 'Unit deleted successfully');
      onAction();
    } catch (error: unknown) {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 custom-card-bg dark:border-white/[0.05]">
                ...
                <TableRow key={unit.id}>
                  ...
                  <TableCell className="px-4 py-3 text-gray-500 text-end text-theme-sm dark:text-gray-400">
                    <TableActions
                      ...
                      onDelete={
                        hasPermission('delete-unit') ? () => setConfirmTarget(unit) : undefined
                      }
                      onRestore={
      ...
      <ConfirmModal
        ...
        title={isTrashed ? 'Permanently Delete Unit' : 'Delete Unit'}
        message={
          isTrashed
