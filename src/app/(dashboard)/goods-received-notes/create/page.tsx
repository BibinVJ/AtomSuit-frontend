...
export default function CreateGoodsReceivedNote() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [vData, iData, ccData, wData, nextNumData, tgData] = await Promise.all([
          ...
          GoodsReceivedNoteService.getNextGrnNumber(),
        ...
        setGrnNumber(nextNumData?.grn_number || nextNumData?.data?.grn_number || '');
  ...
  return (
    <>
      <PageMeta title="Create Goods Received Note" description="Create a new goods received note" />
      <PageBreadcrumb pageTitle="Create Goods Received Note" />

      <form onSubmit={handleSubmit} className="space-y-6">
        {isLoading ? (
          <SkeletonForm rows={3} columns={4} />
        ) : (
          <>
                ...
                {items.map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-[2.5fr_1fr_1fr_1.5fr_1.5fr_0.8fr_1fr] gap-x-4 items-center">
                        {/* Item Select */}
                        <div>
                          <Select
                          ...
                          <div className="flex min-w-0 [&>div]:min-w-0 [&>div]:flex-1">
                            <Input
                              ...
                              decimalPlaces={parseInt(
                                getSetting('general_number_decimal_places', '2')
                              )}
                            />
                            <button
                              type="button"
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <Button type="submit" disabled={isSubmitting}>
                {isSubmitting ? 'Creating...' : 'Create Goods Received Note'}
              </Button>
            </div>
          </>
        )}
