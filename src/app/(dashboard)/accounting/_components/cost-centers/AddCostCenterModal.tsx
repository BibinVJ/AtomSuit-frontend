...
export default function AddCostCenterModal({ isOpen, onClose, onSuccess }: Props) {
    ...
    if (isOpen) {
      const fetchOptions = async () => {
        try {
          ...
          const ccList = (ccData.data || ccData).map(
            (c: { id: string | number; name: string }) => ({ label: c.name, value: String(c.id) })
          );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await addCostCenter(dataToSubmit);
      onSuccess();
  ...
  return (
    <FormModal
      ...
      description="Fill in the details to add a new cost center."
