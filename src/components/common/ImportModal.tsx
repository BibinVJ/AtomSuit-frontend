...
export default function ImportModal({
  ...
  const handleDownloadSample = async () => {
    try {
      const response = await onDownloadSample();
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `sample_${entityName.toLowerCase()}.xlsx`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isApiError(error)) {
        ...
        toast.error(`Failed to import ${entityName.toLowerCase()}`);
      }
    } finally {
      ...
      <div className="relative w-full p-4 bg-white rounded-3xl dark:bg-gray-900">
        <div className="px-2 pr-14">
          ...
          <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
            Upload an Excel file to add multiple {entityName.toLowerCase()} at once.
          </p>
        </div>
