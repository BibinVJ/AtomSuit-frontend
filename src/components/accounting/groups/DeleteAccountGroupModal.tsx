...
export default function DeleteAccountGroupModal({
  isOpen,
  onClose,
  onSuccess,
  accountGroup,
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteAccountGroup(accountGroup.id, force);
      toast.success(
        force
          ? 'Account Group permanently deleted successfully'
      ...
      <div className="relative w-full">
        <div className="p-4 text-center">
          ...
          <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
            {force ? 'Permanently Delete Account Group' : 'Delete Account Group'}
          </h4>
          <p className="mb-6 text-gray-500 dark:text-gray-400">
            Are you sure you want to {force ? 'permanently ' : ''}delete the group &quot;
            {accountGroup?.name}&quot;?{' '}
