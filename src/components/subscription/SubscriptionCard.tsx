...
import { Subscription } from '../../services/TenantSubscriptionService';
...
export default function SubscriptionCard({ subscription, onManage }: Props) {
  ...
  const getStatusColor = () => {
    if (subscription.is_past_due) return 'text-red-600 bg-red-50';
    if (subscription.is_cancelled) return 'text-orange-600 bg-orange-50';
  ...
  const getStatusText = () => {
    if (subscription.is_past_due) return 'Past Due';
  ...
  const getStatusIcon = () => {
    if (subscription.is_past_due) return <AlertTriangle className="w-4 h-4" />;
    if (subscription.is_cancelled) return <AlertTriangle className="w-4 h-4" />;
        ...
        <div className="mb-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-gray-500 dark:text-gray-400">
              per {subscription.items[0].price.recurring.interval}
            </span>
          </div>
        </div>
      )}
