    ...
    <div ref={ref} className="rounded-2xl border border-gray-200 custom-card-bg p-4 md:p-5 flex flex-col h-full w-full dark:border-gray-800">
      ...
      <div className={`flex-1 ${isCompact ? 'flex items-center justify-around' : 'space-y-4'}`}>
        <div className={`flex items-center ${isCompact ? 'flex-col text-center' : 'space-x-2'}`}>
          ...
          <div>
            <p className={`text-gray-600 dark:text-gray-400 ${isCompact ? 'text-xs' : 'text-sm'}`}>New (30 days)</p>
            <p className={`font-semibold text-gray-900 dark:text-white ${isCompact ? 'text-sm' : 'text-lg'}`}>{data.last_30_days}</p>
          </div>
        </div>
        ...
        <div className={`flex items-center ${isCompact ? 'flex-col text-center' : 'space-x-2'}`}>
          ...
          <div>
            <p className={`text-gray-600 dark:text-gray-400 ${isCompact ? 'text-xs' : 'text-sm'}`}>Conversion</p>
            <p className={`font-semibold text-gray-900 dark:text-white ${isCompact ? 'text-sm' : 'text-lg'}`}>{data.conversion_rate}</p>
          </div>
        </div>
      </div>
    </div>
